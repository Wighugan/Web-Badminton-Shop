...
// ===== ĐẾM TỔNG =====
$sql_total = "SELECT COUNT(*) AS total FROM nhanvien $where_sql";
...
// ===== LẤY DỮ LIỆU =====
$sql_data = "SELECT * FROM nhanvien $where_sql ORDER BY id DESC LIMIT ? OFFSET ?";
   ...
   <?php
        ...
        if (!empty($users)) {
            foreach ($users as $row) {
                ...
                $daywork = date("d/m/Y", strtotime($row['daywork']));
                $id = (int)$row['id'];
...
// Nút khóa / mở khóa
echo "<a href='#' onclick='return confirmDelete({$row['id']}, \"nhanvien\")' class='btn btn-sm btn-danger' style='color: red' title='Xóa nhân viên'>";
