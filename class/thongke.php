...
class ThongKe extends QuanLyHeThong {
    protected $data;
    ...
    public function getCustomers($search = '', $start = '', $end = '', $page = 1, $limit = 5) {
        ...
        $sql_count = "SELECT COUNT(DISTINCT khach_hang.MAKH) AS total
                      ...
                      WHERE $where";
        $this->data->select_prepare($sql_count, $types, ...$params);
        $row = $this->data->fetch();
        ...
        $this->data->select_prepare($sql_data, $types . "ii", ...$params_with_limit);
    ...
    public function getTopCustomers($limit = 5) {
        $sql = "SELECT 
                ...
                ORDER BY tongtien DESC
                LIMIT ?";
        $this->data->select_prepare($sql, "i", $limit);
